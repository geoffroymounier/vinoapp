...
export default class MultiSelectList extends React.Component {
  ...
  render() {
      ...
      let data = []
      for (var i in array){
        data.push({title:array[i].label,color:array[i].color,key:i,id:i})
      }
      console.log(data)
    return (
      <Modal
        ...
        >
          ...
          <View style={{backgroundColor:'#FEFDF8'}}>
          <FlatList
            data={data}
            keyExtractor={this._keyExtractor}
            renderItem={this._renderItem}
          />
          <TouchableOpacity onPress={() => this.props.close(this.state.selected)} >
            <Text
                style={{
                textAlign: "center",
                padding: 20,
