...
import {setSearch,resetResults,resetSearch} from 'reduxStore/actions'
...
const Search = ({navigation,route}) => {
  ...
  const triggerSetSearch = (search) => dispatch(setSearch(search))
  const triggerResetSearch = (search) => dispatch(resetSearch(search))
  const triggerfetchSearch = (search) => dispatch(fetchSearch(search))
  ...
  // }
    ...
    const {minYear = 2010,minPrice=0,maxPrice=10, maxYear=2019,minApogee=2018,maxApogee=2020,cuisine_monde,price,favorite,stock,nez,legumes,viandes,poissons,desserts,aperitif,fromages,bouche,appelation,domain,annee,before,apogee,carafage,region,country,pastilles} = search
    const cepage = search.cepage || []
    ...
    return (
      ...
      /> */}
        ...
        <SafeAreaView style={{...styles.container,flexDirection:'row',alignItems:'flex-start',marginBottom:130}}>
              ...
              return (
                <TouchableOpacity key={element}
                  onPress = {()=> {
                    ...
                    triggerSetSearch({color:colorArray})
                  }}
          ...
          <TouchableOpacity onPress={()=>domainRef.current.focus()} style={styles.TouchableOpacity}>
            <View style={{flex:1,flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
              <Text style={{...styles.label}}>Domaine : </Text>
              <TextInput ref={domainRef} value={domain} placeholder={'Non Précisé'} style={styles.title} onChangeText={(domain) => triggerSetSearch({domain})}/>
            </View>
          </TouchableOpacity>

          <TouchableOpacity onPress={()=>navigation.navigate('country',{search:true})} style={styles.TouchableOpacity}>
            <View style={{flex:1,flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
          ...
          <TouchableOpacity onPress={()=>navigation.navigate('region',{search:true})} style={styles.TouchableOpacity}>
            <View style={{flex:1,flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
              <View style={{flex:1,flexDirection:'row',justifyContent:'flex-start',alignItems:'center'}}>
          ...
          <TouchableOpacity onPress={()=>navigation.navigate('appelation',{search:true})} style={styles.TouchableOpacity}>
            <View style={{flex:1,flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
              <View style={{flex:1,flexDirection:'row',justifyContent:'flex-start',alignItems:'center'}}>
          ...
          <TouchableOpacity
            onPress={() => navigation.navigate('cepage',{search:true})}
          ...
          <TouchableOpacity
          onPress = {()=>setSlideActive(slideActive === 'price' ? '' : 'price')}
            style={styles.TouchableOpacity}>
            <View style={{flex:1,flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
              <Text style={{...styles.label,color:slideActive !== 'price' ? 'lightgray' : "#6D6D6D"}}>Prix: </Text>
                <MultiSlider
                  min={0}
                  values={[minPrice,maxPrice]}
                  sliderLength={250}
                  enabledOne={slideActive === 'price'}
                  enabledTwo={slideActive === 'price'}
                  isMarkersSeparated={true}
                  customMarkerLeft={(e) => <CustomMarker
                     active = {slideActive === 'price'}
                     suffix={'€'}
          ...
          <TouchableOpacity
            onPress = {()=>setSlideActive(slideActive === 'year' ? '' : 'year')}
            style={styles.TouchableOpacity}>
            <View style={{flex:1,flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
              <Text style={{...styles.label,color:slideActive !== 'year' ? 'lightgray' : "#6D6D6D"}}>Année : </Text>
                <MultiSlider
                  min={1950}
                  values={[minYear,maxYear]}
                  sliderLength={250}
                  enabledOne={slideActive === 'year'}
                  enabledTwo={slideActive === 'year'}
                  allowOverlap
                  isMarkersSeparated={true}
                  customMarkerLeft={(e) => <CustomMarker
                     active = {slideActive === 'year'}
                   currentValue={e.currentValue}/>
                   }
                 customMarkerRight={(e) => <CustomMarker
                  active = {slideActive === 'year'}
                  ...
                  max={2040}
                  allowOverlap
                  onValuesChangeFinish={(e)=>triggerSetSearch({minYear:e[0],maxYear:e[1]})}
                />
            </View>
          </TouchableOpacity>
          <TouchableOpacity
            onPress = {()=>setSlideActive(slideActive === 'apogee' ? '' : 'apogee')}
            style={styles.TouchableOpacity}>
            <View style={{flex:1,flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
              <Text style={{...styles.label,color:slideActive !== 'apogee' ? 'lightgray' : "#6D6D6D"}}>Apogée : </Text>
                <MultiSlider
                  min={2018}
                  values={[minApogee,maxApogee]}
                  sliderLength={250}
                  enabledOne={slideActive === 'apogee'}
                  enabledTwo={slideActive === 'apogee'}
                  isMarkersSeparated={true}
                  customMarkerLeft={(e) => <CustomMarker
                  active = {slideActive === 'apogee'}
                   currentValue={e.currentValue}/>
                   }
                 customMarkerRight={(e) => <CustomMarker
                  active = {slideActive === 'apogee'}
                  ...
                  max={2025}
                  allowOverlap
                  onValuesChangeFinish={(e)=>triggerSetSearch({minApogee:e[0],maxApogee:e[1]})}
                />
            </View>
          </TouchableOpacity>
          ...
          {Object.keys(accordsValues).map((accords,index) => {
            ...
            return (
              <TouchableOpacity
                onPress={()=>navigation.navigate('accords',{keyValue:accords,search:true})}
          ...
          {Object.keys(json).map((caracts,index) => {
            ...
            return (
              <TouchableOpacity
                onPress={()=>navigation.navigate('aromes',{keyValue:caracts,search:true})}
                key={index} style={{...styles.TouchableOpacity,flexWrap:'wrap',justifyContent:'flex-start',alignItems:'center',paddingVertical:10,marginVertical:5,backgroundColor:'white'}}>
                <View style={{flexDirection:'row'}}>
